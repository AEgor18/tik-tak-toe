    ...
    >
      ...
      >
        <button
          onClick={onClose}
          className="
                        w-8 h-8 flex items-center justify-center bg-white/10 
                        absolute top-0 left-[calc(100%+12px)] rounded hover:bg-white/40 transition-colors"
        >
...
UiModal.Footer = function UiModalFooter({ children, className }) {
  return (
    <div className={clsx("p-6 flex gap-4 justify-end mt-auto", className)}>
      {children}
    </div>
  );
};
